...
/**
 * \brief   Check whether reset is done or not for the DMTimer module.
 *
 * \param   baseAdd       Base Address of the DMTimer Module Register.
 *
 * \return  Returns the status of reset for DMTimer.
 *
 **/
unsigned int DMTimerIsResetDone(unsigned int baseAdd)
{
    /* Return the status of reset for DMTimer */
    return (HWREG(baseAdd + DMTIMER_TIOCP_CFG) & DMTIMER_TIOCP_CFG_SOFTRESET);
}
/**************************** END OF DMTIMER DAL ******************************/
