...
*
* \param  baseAdd        Memory Address of the McSPI instance used.\n
* \param  dataDir        Direction of SPIDAT[1:0] pins.\n
* \param  spiDat         Memory location at which data pin data has to be 
*                        placed.
*
*         'dataPin' can take the following values.\n
...
*         as input.
**/
unsigned int McSPIDataPinRead(unsigned int baseAdd, unsigned int dataPin, 
                              unsigned int *spiDat)
{
    unsigned int retVal = FALSE;

    if(dataPin == (HWREG(baseAdd + MCSPI_SYST) & MCSPI_SYST_SPIDATDIR1))
    {
        retVal = TRUE;
    ...
    else if(dataPin == (HWREG(baseAdd + MCSPI_SYST) & MCSPI_SYST_SPIDATDIR0))
    {
        retVal = TRUE;
...
*         as output.
**/
unsigned int McSPIDataPinWrite(unsigned int baseAdd, unsigned int dataPin,
                               unsigned int spiDat)
{
    unsigned int retVal = FALSE;
