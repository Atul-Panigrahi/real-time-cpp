...
/**
 * \brief   This function enables oscillator compensation mode for the RTC.\n
 *
 * \param   baseAdd   The base address of the RTC instance being used.\n
 *
 * \return  None. 
...
/**
 * \brief   This function disables oscillator compensation mode for the RTC.\n  
 *
 * \param   baseAdd   The base address of the RTC instance being used.\n
 *
 * \return  None.
...
/**
 * \brief   This function sets the SET32COUNTER bit in CTRL register. In turn,
 *          we set the 32KHz counter with the value stored in the compensation 
 *          registers. \n
 *
 * \param   baseAdd   The base address of the RTC instance being used.\n
 *
...
void RTCEnable(unsigned int baseAdd)
{
    ...
    if(RTC_REV_AM1808 == RtcVersionGet())
    {
        ...
        /* Delay further operations by min of 3 peripheral clock times */
        while(delay--);
    }
...
/**
 * \brief  This function sets the meridiem type for the alarm time already
 *         programmed.\n
 *
 * \param   baseAdd      The base address of the RTC instance being used.\n
 * \param   meridiemType  This specifies the meridiem type. This can take one
...
void RTCAlarmHourMeridiemSet(unsigned int baseAdd, unsigned int meridiemType)
{
    ...
    */
    ...
    HWREG(baseAdd + RTC_ALARMHOUR) |= (meridiemType & RTC_HOUR_MERIDIEM);
}
 ...
 * \return  None.
 *
 * \note    When the device is initially powered on, the RTC may issue
...
void RTCSoftwareReset(unsigned int baseAdd)
{
    ...
    {
        HWREG(baseAdd + RTC_OSC) |= RTC_OSC_SWRESET;
        /* Delaying further operations by three peripheral clock cycles. */
        while(delay--);
    }
}
/****************************************************************************/
 ...
 *
 * \param   baseAdd  The base address of the RTC instance being used.
 * \param   modeFlag  This specifies the idle mode to be enabled.
 *
 ...
 * - RTC_IDLEMODE_SMART_IDLE_WAKEUP - to enable Smart Idle Wakeup mode\n
 *
 * \return  None
 */
...
/**
 * \brief  This API is used to control(enable/disable) Alarm events to trigger
 *         a Wake-up signal to the CPU.
 *
 * \param   baseAdd      The base address of the RTC instance being used.\n
 * \param   controlFlag  This specifies whether to enable/disable the feature
 *                       where Alarm events shall trigger a Wake-Up signal to
 *                       the CPU.
 *
...
{
    HWREG(baseAdd + RTC_IRQWAKEEN_0) &= ~(RTC_IRQ_WAKEEN_0_ALARM_WAKEEN);
    HWREG(baseAdd + RTC_IRQWAKEEN_0) |= (controlFlag &
                                         RTC_IRQ_WAKEEN_0_ALARM_WAKEEN);
}

/**
 * \brief  This API is used to control(enable/disable) Timer events to trigger
 *         a Wake-Up signal to the CPU.
 *
 * \param   baseAdd      The base address of the RTC instance being used.\n
 * \param   controlFlag  This specifies whether to enable/disable the feature
...
{
    HWREG(baseAdd + RTC_IRQWAKEEN_0) &= ~(RTC_IRQ_WAKEEN_0_TIMER_WAKEEN);
    HWREG(baseAdd + RTC_IRQWAKEEN_0) |= (controlFlag &
                                         RTC_IRQ_WAKEEN_0_TIMER_WAKEEN);
}

/**
 * \brief   This API controls(enables/disables) Test mode functionality for
 *          the RTC.
 *
 * \param   baseAdd      The base address of the RTC instance being used\n
...
void RTCTestModeControl(unsigned int baseAdd, unsigned int controlFlag)
{
    ...
    /* Setting the TEST_MODE bit of RTC Control Register. */
...
{
    /* Clearing the 32KCLK_EN bit in Oscillator register. */
    HWREG(baseAdd + RTC_OSC) &= ~(RTC_OSC_32KCLK_EN);

    /* Programming the 32KCLK_EN bit in Oscillator register. */
...
/**
 * \brief  This function sets the alarm2 time regiters with the specified
 *         alarm time value. \n
 *
 * \param  baseAdd   The base address of the RTC instance being used.\n
 * \param  alarmTime  The time is specified here and should be in the
